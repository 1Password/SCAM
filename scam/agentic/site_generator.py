...
def _generate_replay_page(
  ...
  <div class="phase-switcher">
    ...
    share_text_baseline = "I just watched an AI agent fail a live security test in real time. Think your AI is safe? See for yourself."
    share_text_skill = "A single system prompt turned an AI agent from a security liability into a defender. Watch the before and after."
    encoded_url = html.escape(page_url)
    pct_url = urllib.parse.quote(page_url, safe="")
    pct_text = urllib.parse.quote(share_text_baseline, safe="")
    pct_text_skill = urllib.parse.quote(share_text_skill, safe="")
...
<style>
  ...
  /* Green success variant for skill replays */
  .skill-cta-success {{
    background: linear-gradient(135deg, #ecfdf5 0%, #f0fdf4 60%, #f5f5f7 100%);
    border-color: #a7f3d0;
  }}
  .skill-cta-success .skill-cta-label {{ color: #065f46; }}
  .skill-cta-btn-success {{
    background: #059669; text-decoration: none; color: #fff;
  }}
  .skill-cta-btn-success:hover {{ background: #047857; color: #fff; text-decoration: none; }}
</style>
</head>
<body>
...
<div class="replay-header-area">
  ...
  <div class="replay-meta-bar">
    ...
    <div class="share-bar">
      ...
      <a class="share-btn" data-share-link="twitter" href="https://twitter.com/intent/tweet?text={pct_text}&url={pct_url}" target="_blank" rel="noopener" title="Share on X">
        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
      </a>
      <a class="share-btn" data-share-link="reddit" href="https://reddit.com/submit?url={pct_url}&title={pct_text}" target="_blank" rel="noopener" title="Share on Reddit">
      ...
      <a class="share-btn" data-share-link="bluesky" href="https://bsky.app/intent/compose?text={pct_text}%20{pct_url}" target="_blank" rel="noopener" title="Share on Bluesky">
...
var __shareUrl = '{pct_url}';
var __shareTexts = {{
  baseline: '{pct_text}',
  skill: '{pct_text_skill}'
}};

function __updateShareLinks(phase) {{
  var txt = __shareTexts[phase] || __shareTexts.baseline;
  document.querySelectorAll('[data-share-link]').forEach(function(a) {{
    var t = a.getAttribute('data-share-link');
    if (t === 'twitter') a.href = 'https://twitter.com/intent/tweet?text=' + txt + '&url=' + __shareUrl;
    else if (t === 'reddit') a.href = 'https://reddit.com/submit?url=' + __shareUrl + '&title=' + txt;
    else if (t === 'bluesky') a.href = 'https://bsky.app/intent/compose?text=' + txt + '%20' + __shareUrl;
  }});
}}

function __copyLinkBtn() {{
  var svgIcon = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>';
  return '<button class="copy-link-btn" onclick="__copyReplayLink(this)" title="Copy link">' + svgIcon + ' Copy link</button>';
}}

function __copyReplayLink(btn) {{
  navigator.clipboard.writeText('{page_url}').then(function() {{
    btn.textContent = 'Copied!';
    setTimeout(function() {{
      btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg> Copy link';
    }}, 1500);
  }});
}}
...
// After playback ends, inject a CTA between the transcript and checkpoints
window.__onPlaybackDone = function() {{
  ...
  if (__currentPhase === 'baseline' && __hasBoth) {{
    // Baseline just finished -- blue CTA nudges toward skill
    cta.innerHTML = '<div class="skill-cta-box">'
      ...
      + '<button class="skill-cta-btn" onclick="switchPhase(\\u0027skill\\u0027, true)">See it again with the skill \\u2192</button>'
      ...
      + '</div>'
      + '</div>';
  }} else if (__currentPhase === 'skill') {{
    // Skill replay finished -- green CTA promotes the skill
    var skTxt = __shareTexts.skill;
    cta.innerHTML = '<div class="skill-cta-box skill-cta-success">'
      + '<div class="skill-cta-label">That\\u2019s the security skill in action.</div>'
      + '<p class="skill-cta-text">'
      + 'One system prompt addition taught the agent to catch what it missed before. '
      + 'The <strong>security expert skill</strong> is free, open source, and works with any model. '
      + 'Install it in seconds.'
      + '</p>'
      + '<div class="skill-cta-actions">'
      + '<a class="skill-cta-btn skill-cta-btn-success" href="https://1password.github.io/SCAM/#skill">Get the skill \\u2192</a>'
      + '<div class="skill-cta-share">'
      + '<span class="skill-cta-share-label">Share this replay</span>'
      + '<a class="share-btn" href="https://twitter.com/intent/tweet?text=' + skTxt + '&url=' + __shareUrl + '" target="_blank" rel="noopener" title="Share on X"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a>'
      + '<a class="share-btn" href="https://reddit.com/submit?url=' + __shareUrl + '&title=' + skTxt + '" target="_blank" rel="noopener" title="Share on Reddit"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0zm5.01 4.744c.688 0 1.25.561 1.25 1.249a1.25 1.25 0 0 1-2.498.056l-2.597-.547-.8 3.747c1.824.07 3.48.632 4.674 1.488.308-.309.73-.491 1.207-.491.968 0 1.754.786 1.754 1.754 0 .716-.435 1.333-1.05 1.604a3.6 3.6 0 0 1 .042.52c0 2.694-3.13 4.876-7.004 4.876-3.874 0-7.004-2.182-7.004-4.876 0-.18.015-.36.043-.52A1.755 1.755 0 0 1 4.028 12c0-.968.786-1.754 1.754-1.754.463 0 .898.196 1.207.49 1.207-.883 2.878-1.43 4.744-1.487l.885-4.182a.342.342 0 0 1 .14-.197.35.35 0 0 1 .238-.042l2.906.617a1.214 1.214 0 0 1 1.108-.701zM9.25 12C8.561 12 8 12.562 8 13.25c0 .687.561 1.248 1.25 1.248.687 0 1.248-.561 1.248-1.249 0-.688-.561-1.249-1.249-1.249zm5.5 0c-.687 0-1.248.561-1.248 1.25 0 .687.561 1.248 1.249 1.248.688 0 1.249-.561 1.249-1.249 0-.687-.562-1.249-1.25-1.249zm-5.466 3.99a.327.327 0 0 0-.231.094.33.33 0 0 0 0 .463c.842.842 2.484.913 2.961.913.477 0 2.105-.056 2.961-.913a.36.36 0 0 0 0-.463.327.327 0 0 0-.462 0c-.547.533-1.684.73-2.512.73-.828 0-1.979-.196-2.512-.73a.326.326 0 0 0-.232-.095z"/></svg></a>'
      + '<a class="share-btn" href="https://bsky.app/intent/compose?text=' + skTxt + '%20' + __shareUrl + '" target="_blank" rel="noopener" title="Share on Bluesky"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 10.8c-1.087-2.114-4.046-6.053-6.798-7.995C2.566.944 1.561 1.266.902 1.565.139 1.908 0 3.08 0 3.768c0 .69.378 5.65.624 6.479.785 2.627 3.6 3.476 6.243 3.226-3.868.577-7.283 2.402-3.7 7.078 3.935 4.842 6.15-.18 6.833-2.928.684 2.748 2.266 7.54 6.833 2.928 3.696-5.166-.073-6.501-3.7-7.078 2.643.25 5.458-.599 6.243-3.226C19.622 9.418 20 4.458 20 3.768c0-.688-.139-1.86-.902-2.203-.66-.299-1.664-.62-4.3 1.24C12.046 4.748 9.087 8.687 8 10.8h4z" transform="translate(2 2) scale(0.833)"/></svg></a>'
      + __copyLinkBtn()
      + '</div>'
      + '</div>'
      + '</div>';
